...
`make format` applies it, `make lint` checks it, and `make style` runs `make
lint` as a gate. This covers both the repo tooling under `scripts/` and the
hand-authored Windmill step scripts under `f/**/*.py`. Type-checking uses
`pyright` (basic mode, with the `f/` rules relaxed for Windmill's
schema-bearing `main()` signatures); it is advisory, surfaced by the editor and
a future CI gate, not by `make style`.
...
never hand-edit them for style. The whitespace and end-of-file checks in `make
style` skip all of `f/`; the Python gate is the only style rule that reaches
`f/`, and only for its `.py`.
