...
"""Wait for one usertests harness to finish on a booted guest, with crash detection.

Polls the guest's journal for the unit `f/usertests/start` started, reading
only entries past start's cursor, so everything seen belongs to this run.
Completion is systemd's own job-outcome record in that stream: PID1 logs
exactly one of `MSG_UNIT_STARTED` ("Finished <unit>", the start job succeeded)
or `MSG_UNIT_FAILED` ("Failed to start <unit>") per start job, and
`MSG_UNIT_STOPPED` when an outside stop ended it. The same records carry the
harness's line-buffered output (the template runs it under
`stdbuf --output=line`) and the unit's process exits (`EXIT_STATUS`), so the
returned `output` is the run-scoped text `f/usertests/collect` parses, and
`exec_status` carries a failing harness's exit status or signal when there is
one (systemd journal-logs successful process exits only at debug level, so a
clean run leaves it empty; the "Finished" outcome already proves the harness
exited 0). The `ExecStart` carries no `-` prefix, so exit codes are REAL: an
assert or sanitizer abort arrives as `result=failed` with `exec_status`
populated, and that IS the test failure.

The unit is `TimeoutStartSec=infinity`, so this poll deadline is the only
...
Equivalent commands:
    ...
    ssh <vm> journalctl --output=json --after-cursor=<cursor> \
        --unit=usertests@<instance>.service
...
def main(
    ...
    def scan(records: list[dict]) -> None:
        """Fold this run's new journal records into the outcome and the output."""
        ...
        for rec in records:
            if rec.get("_SYSTEMD_UNIT") == unit:
                output_lines.append(journal_message(rec))
    ...
    # never-returning harness); abort it rather than leave it spinning.
    if timed_out:
