...
`scripts/` and regenerates with that. `f/common/store`'s `publish_subset` stages that
allowlist and adds it to the store under `kernel-devel-<release>`; the store path is
identical on every host, so a peer can fetch it by release with `nix copy`.
