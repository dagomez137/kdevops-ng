...
.. code-block:: shell
   ...
   machinectl --user terminate <vm>   # emergency kill (immediate, not graceful)
...
.. code-block:: shell
   ...
   systemctl --user restart qemu-system@<vm>               # stop + fresh boot
   systemctl --user stop qemu-system@<vm>                  # graceful powerdown
   systemctl --user kill qemu-system@<vm> --signal=SIGKILL # force, no shutdown
   systemctl --user stop machines.target                   # stop every VM
   systemctl --user reset-failed qemu-system@<vm>          # clear failed state
