...
* ``WORKBENCH_DIR``, ``WORKTREES_DIR``, ``SYSTEM_DIR``, ``MIRRORS_DIR``,
  ``CCACHE_DIR``, ``STORE_INDEX_DIR``, ``WORKERS_DIR``, ``VENDOR_DIR``: the
  build-area paths, each relocatable on its own. See `The workbench`_ for what
  each roots, how they nest, and their defaults.
...
:term:`Worker sandboxes <Worker sandbox>`
(``workers/<id>/``,
