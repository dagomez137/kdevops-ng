...
| **Worktree-group**    | A topic or chain of work within a Workbench (default name `vanilla`; many may exist, e.g. `largeio`). Holds one Worktree per project the topic involves; switchable. The groups default as the Workbench's direct children but relocate as a set on their own. | workbench, default |
| **System workbench**  | The host-local infrastructure singleton (mirrors, bares, ssh key, store), default `system/` under the Workbench but relocatable on its own; its mirrors (the bulky shared object store, default `system/mirror`) relocate apart from it again. User-scoped, sudo-less. | service workbench |
